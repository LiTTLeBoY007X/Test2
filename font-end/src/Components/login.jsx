...
function UserLogin() {
    ...
    const SubmitButton = async (e) => {
        ...
        try {
            // 1. ยิง API Login โดยไม่ต้องใส่ withCredentials แล้ว
            const response = await axios.post(`${API_URL}/user/login`, {
                TelLogin: TelLogin,
                PasswordLogin: Password
            });
            
            // 2. ถ้ามี Token ส่งกลับมา ให้บันทึกลง localStorage
                ...
                <form className='Register-form border-b-[0.5px] flex-initial flex-col items-center w-full' onSubmit={SubmitButton}>
                    <div className='PhoneNumber-Form-Input-Container shadow-[0px_0px_20px_-14px_#000000] mb-3 rounded-xl flex justify-start'>
                        ...
                        <input 
                            ...
                            value={TelLogin} 
                            onChange={(e) => { const onlyNums = e.target.value.replace(/\D/g, '').slice(0, 10); setTelLogin(onlyNums); }} 
                            maxLength={10} 
                            inputMode='numeric'
                        />
                    </div>
                    ...
                    <div className='Button-Form-Input-Container bg-[#11c140] shadow-[0px_0px_20px_-14px_#000000] mb-3 rounded-xl flex justify-start'>
                        <input className='p-2 focus:outline-none text-center w-full text-white cursor-pointer' type='submit' value="เข้าสู่ระบบ" />
                    </div>
                </form>
